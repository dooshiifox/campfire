...
export const user = writable<LoggedInUser | undefined>(
	userStorage ? JSON.parse(userStorage) : undefined
);
...
function addError(message: I18nKey, autoDismiss: boolean = true) {
	errors.update((errors) => {
		...
		errors = errors.filter(
			(error) =>
				error.message !== msg && error.severity !== 'error' && (!error.autoDismiss || !autoDismiss)
		);
...
async function fetchGuilds(accessToken?: string) {
	...
	const fetchedGuilds = await Guild.getJoined(accessToken)
		.onBadAuthToken(() => {
